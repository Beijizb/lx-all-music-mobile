...
const utf8Encode = (str: string) => {
  ...
  for (let n = 0; n < normalized.length; n++) {
    ...
    if (c < 128) {
      ...
      bytes.push((c >> 6) | 192)
      bytes.push((c & 63) | 128)
    } else if (c >= 0xd800 && c <= 0xdbff && n + 1 < normalized.length) {
      const c2 = normalized.charCodeAt(n + 1)
...
const convertToWordArray = (bytes: number[]) => {
  const numberOfWordsTempOne = bytes.length + 8
  ...
  while (byteCount < bytes.length) {
    ...
    wordArray[wordCount] = wordArray[wordCount] | (bytes[byteCount]! << bytePosition)
    byteCount++
  }
  ...
  wordArray[numberOfWords - 1] = bytes.length >>> 29
  return wordArray
}
